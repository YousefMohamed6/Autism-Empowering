...
class _DoctorRequestsScreenState extends State<DoctorRequestsScreen> {
  ...
  void _acceptRequest(
      String requestId, String patientFcmToken, String doctorName) async {
    await updateFollowRequestStatus(
        requestId, 'accepted', patientFcmToken, doctorName);
  }

  void _denyRequest(
      String requestId, String patientFcmToken, String doctorName) async {
    await updateFollowRequestStatus(
        requestId, 'rejected', patientFcmToken, doctorName);
  }
                  ...
                  children: [
                    IconButton(
                      ...
                      onPressed: () => _acceptRequest(request.requestId,
                          request.patientFcmToken, request.doctorName),
                    ),
                    IconButton(
                      icon: const Icon(Icons.close, color: Colors.red),
                      onPressed: () => _denyRequest(request.requestId,
                          request.patientFcmToken, request.doctorName),
                    ),
                  ],
                ),
...
Future<void> updateFollowRequestStatus(String requestId, String status,
    String patientFcmToken, String doctorName) async {
  try {
    ...
    OneSignalService.sendNotification(
      ...
      message: '',
      senderName: 'Dr. $doctorName $status Your Request',
    );
