import 'package:autism_empowering/Model/content_modal.dart';
        ...
        body: Container(
          decoration: BoxDecoration(
            image: DecorationImage(
              fit: BoxFit.cover,
              opacity: 0.87,
              image: AssetImage(GameData.image),
            ),
          ),
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: GameData.list.map((e) {
                          int i = list2.indexOf(e);
                          return GameData.list[i].isDropped == true
                              ? Container(
                                  height: height,
                                )
                              : Draggable(
                                  data: GameData.list[i].value,
                                  childWhenDragging: Container(
                                    height: height,
                                    padding: const EdgeInsets.all(10),
                                    child: Image.asset(
                                      GameData.list[i].image,
                                    ),
                                  ),
                                  feedback: SizedBox(
                                      height: height,
                                      child:
                                          Image.asset(GameData.list[i].image)),
                                  child: Container(
                                    height: height,
                                    padding: const EdgeInsets.all(10),
                                    child: Image.asset(
                                  ...
                                  onAcceptWithDetails: (data) {
                                    if (list2[i].value == data) {
                                      setState(() {
                                        ...
                                        score += 10;
                                        gameOver++;
                                        if (gameOver == GameData.list.length) {
                                          showDialog(
                                              context: context,
